# @shapeshift-labs/frontier-lang-rust

Rust projection adapter for Frontier Lang semantic documents.
...
const { code, sourceMap, ast } = emitRustWithSourceMap(document, {
  sourcePath: 'todo.frontier',
  targetPath: 'src/todo.rs',
  semanticIndexId: 'semantic_index_todo'
});

const evidence = createRustSemanticMergeEvidence(
  'pub struct Todo;\\nimpl Todo { pub fn save(&self) {} }\\n',
  { sourcePath: 'src/todo.rs' }
);
```
...
`createRustSemanticMergeEvidence` extracts dependency-light Rust source records for top-level items and impl methods. It deliberately fails closed for macro expansion, conditional compilation, unsafe Rust, FFI, trait resolution, borrow checking, and type checking unless stronger host compiler evidence is attached.
...
These are Frontier-only package measurements for @shapeshift-labs/frontier-lang-rust. They exercise the package's own parser, checker, compiler, projection, CLI, fuzz, or semantic-kernel surface without making competitor comparison claims.
